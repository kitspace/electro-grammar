...
function matchResistor(c) {
    return resistors.reduce(function(prev, cpl) {
        ...
        const power_rating =
            c.power_rating == null || cpl.power_rating >= c.power_rating
...
function matchCapacitor(c) {
    return capacitors.reduce(function(prev, cpl) {
        ...
        const characteristic =
            c.characteristic == null || cpl.characteristic === c.characteristic
        const tolerance = c.tolerance == null || cpl.tolerance <= c.tolerance
        const voltage_rating =
            c.voltage_rating == null || cpl.voltage_rating >= c.voltage_rating
        if (
            capacitance &&
            size &&
            characteristic &&
